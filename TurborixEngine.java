...
public class TurborixEngine extends Object implements Runnable {
   ...
   public static final String PIUE  = "PORTINUSEEXCEPTION";
   ...
   public TurborixEngine(TurborixUI t, String portname, String altportname, String libname) {
      super();
      tui = t;
      loadLibrary(libname);
      ...
      try {
         ...
         if (e.toString().toUpperCase().indexOf(PIUE) != -1) {
             tui.setMessage("Connect to '" + portname + "' failed - " + e);
             return;
         }
         tui.setMessage("Connect to '" + portname + "' failed - " + e);
      }
      if (!portok) {
          try {
             ...
             if (e1.toString().toUpperCase().indexOf(PIUE) != -1) {
                tui.setMessage("Connect to '" + altportname + "' failed - " + e1);
                return;
             }
             tui.setMessage("Connect '" + portname + "' and '" + altportname + "' failed - " + e1);
         }
      }
   }

   public void loadLibrary(String l) {
       try {
          System.loadLibrary(l);
          loaded = true;
       } catch (java.lang.UnsatisfiedLinkError e) {
          tui.setMessage("Error loading " + l + " library - " + e);
       }
   }
   ...
   void connect ( String portName ) throws Exception {
        ...
        if ( portIdentifier.isCurrentlyOwned()) {
            tui.setMessage("Error: Port is currently in use");
            connected = false;
        } else {
            CommPort commPort = portIdentifier.open(this.getClass().getName(),PORTID);
            if ( commPort instanceof SerialPort ) {
                ...
                tui.setMessage("Error: Only serial ports are handled by this program.");
                connected = false;
            }
        }     
   }
